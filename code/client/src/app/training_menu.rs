...
impl TrainingMenu {
    pub fn new() -> Self {
        ...
        // Clone the spawn points first to avoid borrowing game_engine while mutating it
...
impl View for TrainingMenu {
    ...
    fn draw(&mut self, _ctx: &AppContext) {
        ...
        if self.bot_brain.is_none() {
            ...
            ui::Text::new_scaled(50).draw("DRAW", screen_width() / 2., screen_height() / 2.);
        } else if blue_count == 0 {
            ui::Text::new_scaled(50).draw("RED WINS", screen_width() / 2., screen_height() / 2.);
        } else if red_count == 0 {
            ui::Text::new_scaled(50).draw("BLUE WINS", screen_width() / 2., screen_height() / 2.);
        }
    }
