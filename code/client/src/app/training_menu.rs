...
use crate::ui;
use crate::ui::{Button, BUTTON_H, BUTTON_W, CANONICAL_SCREEN_WIDTH, CANONICAL_SCREEN_HEIGHT};
...
use macroquad::miniquad::gl::{glScissor, glEnable, glDisable, GL_SCISSOR_TEST};
...
impl TrainingMenu {
    ...
    fn bot_action_to_input(actions: &[f32], ctx: &BotContext) -> InputPayload {
        ...
        let world_move = Vec2::new(move_fwd * cos - move_side * sin, move_fwd * sin + move_side * cos);
        let world_aim_dir = Vec2::new(aim_fwd * cos - aim_side * sin, aim_fwd * sin + aim_side * cos);
        let final_aim_dir = if world_aim_dir.length_squared() < 0.001 { Vec2::new(cos, sin) } else { world_aim_dir.normalize() };
        let aim_pos = ctx.me.position + (final_aim_dir * 100.0);

        InputPayload { move_axis: world_move, aim_pos, shoot: shoot_val > 0.0 }
    }
    ...
    fn draw_game(game_engine: &GameEngine) {
        ...
        for player in &game_engine.players {
            ...
            draw_line(
                transform_x(player.position.x), transform_y(player.position.y),
                transform_x(player.position.x + aim_dir.x * 30.0), transform_y(player.position.y + aim_dir.y * 30.0),
                scale(3.0), DARKGRAY,
            );
            let hp_pct = player.health / 100.0;
            draw_rectangle(transform_x(player.position.x - 20.), transform_y(player.position.y - 30.), scale(40. * hp_pct), scale(5.), GREEN);
        }

        for proj in &game_engine.projectiles {
            draw_circle(transform_x(proj.position.x), transform_y(proj.position.y), scale(proj.radius), YELLOW);
        }
    }
}
...
impl View for TrainingMenu {
    fn update(&mut self, _ctx: &mut AppContext) -> Transition {
        ...
        match &mut self.state {
            ...
            TrainingState::Playing { game_engine, bot_brain, human_id, .. } => {
                ...
                if let Some(hid) = human_id {
                    ...
                    let input = InputPayload {
                        move_axis: {
                            ...
                            if is_key_down(KeyCode::W) { axis.1 -= 1.0; }
                            if is_key_down(KeyCode::S) { axis.1 += 1.0; }
                            if is_key_down(KeyCode::A) { axis.0 -= 1.0; }
                            if is_key_down(KeyCode::D) { axis.0 += 1.0; }
    ...
    fn draw(&mut self, _ctx: &AppContext) {
        ...
        match &mut self.state {
            TrainingState::ModelSelect { files, error_msg, scroll } => {
                ...
                let (scale, x_off, y_off) = ui::calc_transform(CANONICAL_SCREEN_WIDTH, CANONICAL_SCREEN_HEIGHT);
                ...
                for file in files.iter() {
                    ...
                    if item_y + BUTTON_H > list_start_y && item_y < list_end_y {
                        if Button::default().draw_centered(x_mid, item_y, BUTTON_W * 1.5, BUTTON_H, Some(file)).poll() {
                ...
                // Back Button (Fixed)
                if Button::default().draw_centered(x_mid, back_button_y, BUTTON_W, BUTTON_H, Some("Back")).poll() {
                     self.back_clicked = true;
                }

                if let Some(fname) = picked_file {
                    let full_path = format!("assets/models/{}", fname);
                    let load_name = if fname.ends_with(".bin") { &full_path[..full_path.len() - 4] } else { &full_path };
                    let recorder = BinFileRecorder::<FullPrecisionSettings>::default();
                    if let Ok(brain) = BotBrain::<ClientBackend>::new(&Default::default()).load_file(load_name, &recorder, &Default::default()) {
                         next_state = Some(TrainingState::ModeSelect { model_name: fname, brain });
                    }
                }
            }
            ...
            TrainingState::ModeSelect { model_name, brain } => {
                 ...
                 layout.add(50.);
                 ui::Text::new_scaled(ui::TEXT_MID).draw(&format!("Model: {}", model_name), x_mid, layout.next());
                 layout.add(50.);

                 if Button::default().draw_centered(x_mid, layout.next(), BUTTON_W * 1.5, BUTTON_H, Some("Spectator (4v4)")).poll() {
                      next_state = Some(Self::init_game(brain.clone(), TrainingMode::Spectator));
                 }
                 layout.add(BUTTON_H);

                 if Button::default().draw_centered(x_mid, layout.next(), BUTTON_W * 1.5, BUTTON_H, Some("Play Solo vs 4 Bots")).poll() {
                      next_state = Some(Self::init_game(brain.clone(), TrainingMode::HumanVsAi));
                 }
                 layout.add(BUTTON_H);
                                  
                 layout.add(20.);
                 if Button::default().draw_centered(x_mid, layout.next(), BUTTON_W, BUTTON_H, Some("Back")).poll() {
                     self.back_clicked = true;
                 }
            }

            TrainingState::Playing { game_engine, mode, .. } => {
                Self::draw_game(game_engine);
                let mode_str = match mode {
                    TrainingMode::Spectator => "SPECTATOR",
                    TrainingMode::HumanVsAi => "PLAYING",
                };
                ui::Text::new_scaled(20).draw(&format!("{} | Reset: R | Exit: ESC", mode_str), screen_width() / 2., 30.);
            }
        }
...
mod tests {
    ...
    fn test_refresh_file_list() {
        ...
        if let TrainingState::ModelSelect { files, .. } = menu.state {
             assert!(files.contains(&test_filename.to_string()), "File list should contain created test file");
        } else {
    ...
    fn test_init_game_spectator() {
        ...
        if let TrainingState::Playing { game_engine, mode, human_id, .. } = state {
